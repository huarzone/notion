...
论文页面按 **Lilian Weng 技术博客 + 深度解析页** 风格撰写，参考：

- https://lilianweng.github.io/posts/2021-07-11-diffusion-models/
- https://lilianweng.github.io/posts/2023-06-23-agent/
- https://lilianweng.github.io/posts/2024-07-07-hallucination/

核心目标：**人类友好、可连续阅读、像讲课一样解释论文**，不是字段表、摘要翻译或 bullet 清单。页面应让读者从“没读过论文”推进到“能复述核心机制、知道边界和复现风险”。

### 深度解析页结构

优先采用以下结构，不必机械照搬，但信息密度要对齐：

1. **Hero / 快速定位**：标题、副标题、年份 / 机构 / venue、核心 tags、Paper / Code / Project 链接
2. **架构总览**：先放一张框架图或自绘流程图，给读者全局地图
3. **目录 / 阅读路线**：用 6-10 个小节告诉读者本文会回答哪些问题
4. **核心问题**：从真实痛点、反直觉现象或部署矛盾切入，而不是复述 abstract
5. **背景地图**：解释任务背景、已有方法为什么不够、论文站在哪个位置
6. **方法深拆**：逐个讲关键模块，每个模块回答“为什么需要、怎么做、代价是什么”
7. **数据流 / 训练流 / 推理流**：用伪代码、流程图式列表或表格把输入到输出讲清楚
8. **模型 / 场景特定分析**：如果论文比较多个模型、任务或配置，要拆开说明差异，不要混写
9. **实验与证据**：只突出能支撑主张的关键指标；完整表格放 `<details>`，避免指标刷屏
10. **设计洞察**：总结 3-6 条真正可迁移的系统设计原则
11. **Q&A / 常见误解**：用问答解释读者最容易卡住的问题
12. **代码 / 复现指南**：开源论文写运行入口、关键配置、排错；闭源论文写已披露信息与缺失信息
13. **局限与 Take-away**：明确论文没解决什么，以及对后续系统设计的启发
...
- **具体例子优先**：用论文中的模型名、阈值、数据规模、速度 / 质量数字解释观点
- **对比优先**：多用“人工 vs 自动”“baseline vs ours”“不同模型 / 场景策略差异”建立心智模型
- **机制优先**：不要只说“使用 X 模块”，要解释 X 解决了什么冗余 / 对齐 / 记忆 / 控制问题
- **路径优先**：复杂系统必须讲清输入、状态、模块交互、输出，以及失败时如何回退
- **复现优先**：开源则给代码入口和关键配置；未开源则明确缺失项，禁止脑补实现细节
...
- 代码块：可用于伪代码、流程摘要、配置示例、排错清单，让复杂机制更直观
- 公式：只放支撑理解的关键公式；公式前后必须解释变量和直觉，避免裸公式
- 图片：引用前后要说明“这张图看什么”；优先使用论文原图，无法获取原图时标明 PDF 裁剪 / MinerU 派生
...
- 不要只有“方法 / 实验 / 局限”三段式，必须补背景、机制、数据流、设计洞察和 Q&A
